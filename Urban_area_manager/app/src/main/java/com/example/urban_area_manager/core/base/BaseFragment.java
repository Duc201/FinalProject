...
public abstract class BaseFragment<VB extends ViewBinding,VM extends BaseViewModel> extends Fragment implements BaseBehavior {
    ...
    protected void openFragment(BaseFragment<?,?> cls , int frame){
        FragmentManager fragmentManager = getParentFragmentManager();
        FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
        fragmentTransaction.replace(frame, cls);
        fragmentTransaction.addToBackStack(null);
        fragmentTransaction.commit();
    }
    protected void openFragment(BaseFragment<?,?> cls , int frame, Bundle bundle){
        cls.setArguments(bundle);
        getParentFragmentManager().beginTransaction().replace(frame, cls).addToBackStack(null).commit();
    }
    protected void addFragment(BaseFragment<?,?> cls , int frame){
        getParentFragmentManager().beginTransaction().add(frame, cls).addToBackStack(null).commit();
    }
    protected void addFragment(BaseFragment<?,?> cls , int frame, Bundle bundle){
        cls.setArguments(bundle);
        getParentFragmentManager().beginTransaction().add(frame, cls).addToBackStack(null).commit();
    }
